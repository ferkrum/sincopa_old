...
static int barkSpec_nearest_bin_index(float target, float *bin_freqs)
{
	int i, nearest;
	float after, before;
...
static float barkSpec_bark2freq(float bark)
{
	float freq;
...
static void barkSpec_make_filterbank(t_barkSpec *x, float *bark_spaced_freqs)
{	
	...
	for(i=1; i<=x->num_filters; i++)
	{
		...
		start_index = barkSpec_nearest_bin_index(bark_spaced_freqs[i-1], bin_freqs);
		peak_index = barkSpec_nearest_bin_index(bark_spaced_freqs[i], bin_freqs);
		finish_index = barkSpec_nearest_bin_index(bark_spaced_freqs[i+1], bin_freqs);
...
static void barkSpec_blackman_window(int n, t_sample *in)
{
	int i;
...
static void barkSpec_cosine_window(int n, t_sample *in)
{
	int i;
...
static void barkSpec_hamming_window(int n, t_sample *in)
{	
	int i;
...
static void barkSpec_hann_window(int n, t_sample *in)
{
	int i;
...
static void barkSpec_realfft_unpack(int n, int n_half, t_sample *input, t_sample *imag)
{
	int i, j;
...
static void barkSpec_abs(int n, t_sample *in1, t_sample *in2)
{
	while (n--)
    {   
...
static void barkSpec_square(int n, t_sample *in)
{
	while (n--)
    {   
	    *in *= *in;	        
...
static void barkSpec_filterbank_multiply(t_sample *in, int normalize, t_filter *x_filterbank, t_indices *x_indices, int num_filters)
{
	int i, j, k;
...
static void barkSpec_create_filterbank(t_barkSpec *x, t_floatarg bs, t_floatarg suppress_post)
{
	...
	if(bs >= 0.02 && bs <= 10.0)
	{
		...
		{
			bark_spaced_freqs[i] = barkSpec_bark2freq(bark_sum);
			bark_sum += x->bark_spacing;
		};
		...
		barkSpec_make_filterbank(x, bark_spaced_freqs);
...
static void barkSpec_analyze(t_barkSpec *x, t_floatarg start, t_floatarg n)
{
	...
	old_window = x->window;
	x->window = window;
	barkSpec_create_filterbank(x, x->bark_spacing, 1);
	...
	{
		case 0:
			barkSpec_blackman_window(length_samp, x->signal_R);
			break;
		case 1:
			barkSpec_cosine_window(length_samp, x->signal_R);
			break;
		case 2:
			barkSpec_hamming_window(length_samp, x->signal_R);
			break;
		case 3:
			barkSpec_hann_window(length_samp, x->signal_R);
			break;
		default:
			break;
	};
	...
	barkSpec_realfft_unpack(window, window_half, x->signal_R, signal_I);
	barkSpec_abs(window_half, x->signal_R, signal_I);

// power spectrum sometimes generates lower scores than magnitude. make it optional.
	if(x->power_spectrum)
		barkSpec_square(window_half, x->signal_R);

	barkSpec_filterbank_multiply(x->signal_R, x->normalize, x->x_filterbank, x->x_indices, x->num_filters);
...
static void barkSpec_bang(t_barkSpec *x)
{
	...
	barkSpec_analyze(x, start_samp, window);
...
static void barkSpec_hat(t_barkSpec *x, t_floatarg f, t_floatarg g)
{
	int i;
...
static void barkSpec_window(t_barkSpec *x, t_floatarg w)
{
	...
	else
	{
		...
		barkSpec_create_filterbank(x, x->bark_spacing, 0);
...
static void barkSpec_window_function(t_barkSpec *x, t_floatarg f)
{
...
static void barkSpec_power_spectrum(t_barkSpec *x, t_floatarg spec)
{
...
static void barkSpec_debug(t_barkSpec *x, t_floatarg f)
{
	if((int)f < 0)
		x->debug = 0;
	else if((int)f > 2)
...
static void *barkSpec_new(t_symbol *s, t_floatarg bark_spacing)
{
	...
	x->power_spectrum = 0; // choose mag (0) or power (1) spec in the barkSpec computation
...
void barkSpec_setup(void)
{
	...
	class_addmethod(
		barkSpec_class, 
        (t_method)barkSpec_window_function,
	...
	class_addmethod(
		barkSpec_class, 
        (t_method)barkSpec_power_spectrum,
