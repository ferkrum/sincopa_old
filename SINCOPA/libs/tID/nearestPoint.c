...
{
	int i, point_ind, dimensions;

	point_ind = x->num_points;
	dimensions = argc;
	s=s; // to get rid of 'unused variable' warning

	if(dimensions == x->dimensions)
	{
		x->x_coordinates = (t_coordinate *)t_resizebytes(x->x_coordinates, x->num_points * sizeof(t_coordinate), (x->num_points+1) * sizeof(t_coordinate));
		
		x->x_coordinates[point_ind].coordinate = (t_float *)getbytes(0);
		x->x_coordinates[point_ind].coordinate = (t_float *)t_resizebytes(x->x_coordinates[point_ind].coordinate, 0, dimensions * sizeof(t_float));

		
		x->num_points++;
				
		for(i=0; i<dimensions; i++)
			x->x_coordinates[point_ind].coordinate[i] = atom_getfloat(argv+i);
	}
	else
...
static void nearestPoint_nearest(t_nearestPoint *x, t_symbol *s, int argc, t_atom *argv)
{
	...
	{		
		...
		if(dimensions == x->dimensions)
		{
			...
			outlet_float(x->nearest_dist, best);
...
//     else
//     	error("nearestPoint: no coordinates have been added.");
...
{
	x->x_coordinates = (t_coordinate *)t_resizebytes(x->x_coordinates, x->num_points * sizeof(t_coordinate), 0);
	x->num_points = 0;
}
...
static void *nearestPoint_new(t_float dim)
{
    ...
    x->x_coordinates = (t_coordinate *)getbytes(0);
	...
	x->x_input = (t_float *)t_resizebytes(x->x_input, 0, x->dimensions * sizeof(t_float));
...
{
   t_freebytes(x->x_coordinates, x->num_points*x->dimensions*sizeof(t_float));
