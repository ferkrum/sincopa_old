...
static void specRolloff_tilde_hann(int n, t_sample *in, float *hann)
{	
	while (n--)
    {
    	*in = *in * *hann;
...
static void specRolloff_tilde_realfft_unpack(int n, int n_half, t_sample *input, t_sample *imag)
{
	int i, j;
...
static void specRolloff_tilde_abs(int n, t_sample *in1, t_sample *in2)
{
	while (n--)
    {   
...
static void specRolloff_tilde_normal(int n, t_sample *in)
{
	int i;
...
static void specRolloff_tilde_bang(t_specRolloff_tilde *x)
{
	...
	specRolloff_tilde_hann(window, signal_R, x->hann);
	mayer_realfft(window, signal_R);
	specRolloff_tilde_realfft_unpack(window, window_half, signal_R, signal_I);
	specRolloff_tilde_abs(window_half, signal_R, signal_I);
	
// 	if(x->normalize)
// 		specRolloff_tilde_normal(window_half, signal_R);

	energy_target=0;
	
	for(i=0; i<window_half; i++)
		energy_target += signal_R[i];
...
static void specRolloff_tilde_concentration(t_specRolloff_tilde *x, t_floatarg f)
{
	if(f<0 || f>1.0)
...
static void specRolloff_tilde_window(t_specRolloff_tilde *x, t_floatarg f)
{
	int i, isPow2;
...
static void specRolloff_tilde_overlap(t_specRolloff_tilde *x, t_floatarg f)
{
	int i;
...
static void specRolloff_tilde_normalize(t_specRolloff_tilde *x, t_floatarg norm)
{
	if(norm<0)
		x->normalize = 0;
	else if (norm>1)
...
// compare n to stored n and recalculate filterbank if different
	if( sp[0]->s_sr != x->sr || sp[0]->s_n != x->n )
	{
    	...
    	post("specRolloff~: window size: %i. overlap: %i. sampling rate: %i, block size: %i", (int)x->window, x->overlap, (int)(x->sr/x->overlap), (int)x->n);
	};
};
...
void specRolloff_tilde_setup(void)
{
	...
	class_addmethod(
		specRolloff_tilde_class, 
        (t_method)specRolloff_tilde_normalize, 
		gensym("normalize"),
		A_DEFFLOAT,
		0
	);
