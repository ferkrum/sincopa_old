...
bfcc~ - A Bark Frequency Cepstral Analysis external.
...
static int bfcc_tilde_nearest_bin_index(float target, float *bin_freqs)
{
	int i, nearest;
	float after, before;
...
static float bfcc_tilde_bark2freq(float bark)
{
	float freq;
...
static void bfcc_tilde_make_filterbank(t_bfcc_tilde *x, float *bark_spaced_freqs)
{	
	...
	for(i=1; i<=x->num_filters; i++)
	{
		...
		start_index = bfcc_tilde_nearest_bin_index(bark_spaced_freqs[i-1], bin_freqs);
		peak_index = bfcc_tilde_nearest_bin_index(bark_spaced_freqs[i], bin_freqs);
		finish_index = bfcc_tilde_nearest_bin_index(bark_spaced_freqs[i+1], bin_freqs);
...
static void bfcc_tilde_DSP_window(int n, t_sample *in, float *window_func)
{	
	while (n--)
    {
    	*in *= *window_func;
...
static void bfcc_tilde_realfft_unpack(int n, int n_half, t_sample *input, t_sample *imag)
{
	int i, j;
...
static void bfcc_tilde_abs(int n, t_sample *in1, t_sample *in2)
{
	while (n--)
    {   
...
static void bfcc_tilde_square(int n, t_sample *in)
{
	while (n--)
    {   
	    *in *= *in;	        
...
static void bfcc_tilde_compute_bfccs(float *bfcc, t_sample *in, int num_filters)
{
	int i, k;
...
static void bfcc_tilde_create_filterbank(t_bfcc_tilde *x, t_floatarg bs)
{
	...
	if(bs >= 0.02 && bs <= 10.0)
	{
		...
		{
			bark_spaced_freqs[i] = bfcc_tilde_bark2freq(bark_sum);
			bark_sum += x->bark_spacing;
		};
		...
		bfcc_tilde_make_filterbank(x, bark_spaced_freqs);
...
static void bfcc_tilde_hat(t_bfcc_tilde *x, t_floatarg f, t_floatarg g)
{
	int i;
...
static void bfcc_tilde_overlap(t_bfcc_tilde *x, t_floatarg f)
{
	if((int)f > 0)
		x->overlap = (int)f;
	else
...
static void bfcc_tilde_window_function(t_bfcc_tilde *x, t_floatarg f)
{
...
static void bfcc_tilde_power_spectrum(t_bfcc_tilde *x, t_floatarg spec)
{
...
static void bfcc_tilde_debug(t_bfcc_tilde *x, t_floatarg f)
{
	if((int)f < 0)
		x->debug = 0;
	else if((int)f > 2)
...
static void *bfcc_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
	...
	// x->bfcc is resized in bfcc_tilde_create_filterbank
    ...
    post("bfcc~: window size: %i", (int)x->window);
...
// compare n to stored n and recalculate filterbank if different
	if( sp[0]->s_sr != x->sr || sp[0]->s_n != x->n )
	{
    	...
    	post("bfcc~: window size: %i. overlap: %i. sampling rate: %i, block size: %i", (int)x->window, x->overlap, (int)(x->sr/x->overlap), (int)x->n);
	};
...
void bfcc_tilde_setup(void)
{
	...
	class_addmethod(
		bfcc_tilde_class, 
        (t_method)bfcc_tilde_window_function,
	...
	class_addmethod(
		bfcc_tilde_class, 
        (t_method)bfcc_tilde_power_spectrum,
