...
static void specCentroid_tilde_abs(int n, t_sample *in1, t_sample *in2)
{
	while (n--)
    {   
...
static void specCentroid_tilde_normal(int n, t_sample *in)
{
	int i;
...
static void specCentroid_tilde_normalize(t_specCentroid_tilde *x, t_floatarg norm)
{
	if(norm<0)
		x->normalize = 0;
	else if (norm>1)
...
void specCentroid_tilde_setup(void)
{
	...
	class_addmethod(
		specCentroid_tilde_class, 
        (t_method)specCentroid_tilde_normalize, 
		gensym("normalize"),
		A_DEFFLOAT,
		0
	);
