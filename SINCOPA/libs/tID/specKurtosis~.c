...
static void specKurtosis_tilde_hann(int n, t_sample *in, float *hann)
{	
	while (n--)
    {
    	*in = *in * *hann;
...
static void specKurtosis_tilde_realfft_unpack(int n, int n_half, t_sample *input, t_sample *imag)
{
	int i, j;
...
static void specKurtosis_tilde_abs(int n, t_sample *in1, t_sample *in2)
{
	while (n--)
    {   
...
static void specKurtosis_tilde_normal(int n, t_sample *in)
{
	int i;
...
static void specKurtosis_tilde_bang(t_specKurtosis_tilde *x)
{
    ...
    float dividend, divisor, centroid, std, kurtosis;
	...
	specKurtosis_tilde_hann(window, signal_R, x->hann);
	mayer_realfft(window, signal_R);
	specKurtosis_tilde_realfft_unpack(window, window_half, signal_R, signal_I);
	specKurtosis_tilde_abs(window_half, signal_R, signal_I);
	
// 	if(x->normalize)
// 		specKurtosis_tilde_normal(window_half, signal_R);
	...
	for(i=0; i<window_half; i++)
		dividend += pow((x->bin_freqs[i] - centroid), 2) * signal_R[i];
	...
	for(i=0; i<window_half; i++)
		dividend += pow((x->bin_freqs[i] - centroid), 4) * signal_R[i];
...
static void specKurtosis_tilde_window(t_specKurtosis_tilde *x, t_floatarg f)
{
	int i, isPow2;
...
static void specKurtosis_tilde_overlap(t_specKurtosis_tilde *x, t_floatarg f)
{
	int i;
...
static void specKurtosis_tilde_normalize(t_specKurtosis_tilde *x, t_floatarg norm)
{
	if(norm<0)
		x->normalize = 0;
	else if (norm>1)
...
// compare n to stored n and recalculate filterbank if different
	if( sp[0]->s_sr != x->sr || sp[0]->s_n != x->n )
	{
    	...
    	post("specKurtosis~: window size: %i. overlap: %i. sampling rate: %i, block size: %i", (int)x->window, x->overlap, (int)(x->sr/x->overlap), (int)x->n);
	};
};
...
void specKurtosis_tilde_setup(void)
{
	...
	class_addmethod(
		specKurtosis_tilde_class, 
        (t_method)specKurtosis_tilde_normalize, 
		gensym("normalize"),
		A_DEFFLOAT,
		0
	);
